...
// Image   Button
+(UIButton*)makeBtn_Image:(NSString*)imageName ToView:(UIView*)superView{
    UIButton * btn = [UIFactory makeBtn];
...
+(UIButton*)makeBtn_BgImage:(NSString *)bgImageName ToView:(UIView*)superView{
    UIButton * btn = [UIFactory makeBtn];
...
//创建 图片按钮 到父视图
+(UIButton*)makeBtn_ImageName:(NSString*)imageName ToView:(UIView*)superView{
    UIButton * btn = [UIFactory makeButton_ToView:superView];
    [btn setImage:[UIImage imageNamed:imageName] forState:UIControlStateNormal];
    return btn;
}
